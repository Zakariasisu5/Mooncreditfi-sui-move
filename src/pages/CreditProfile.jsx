...
const CreditProfile = () => {
  ...
  const walletDataForAI = useMemo(() => {
    ...
    return {
      ...
      walletAge: Math.max(1, totalLoans * 2),
      totalVolume: (creditProfile.totalBorrowed + creditProfile.totalRepaid) * 1000,
      ...
      totalBorrowed: creditProfile.totalBorrowed * 1000,
      totalRepaid: creditProfile.totalRepaid * 1000,
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        />
        <StatsCard title="Available Credit" value={`${creditProfile.availableCredit.toFixed(4)} SUI`} description="Ready to borrow" icon={DollarSign} />
        <StatsCard title="Utilized Credit" value={`${creditProfile.utilizedCredit.toFixed(4)} SUI`} description="Currently borrowed" icon={CreditCard} />
        <StatsCard title="Utilization Rate" value={`${utilizationRate.toFixed(1)}%`} description="Credit utilization" icon={User} />
      </div>

      {/* AI Credit Analysis Section */}
      <AICreditAnalysis walletData={walletDataForAI} onAnalysisComplete={setAiAnalysis} />
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>Used: {creditProfile.utilizedCredit.toFixed(4)} SUI</span>
                <span>Available: {creditProfile.availableCredit.toFixed(4)} SUI</span>
              </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-muted/50 rounded-lg">
                ...
                <p className="font-bold">{creditProfile.totalBorrowed.toFixed(4)} SUI</p>
              </div>
              <div className="p-3 bg-muted/50 rounded-lg">
                <p className="text-xs text-muted-foreground">Total Repaid</p>
                <p className="font-bold text-green-500">{creditProfile.totalRepaid.toFixed(4)} SUI</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              {creditProfile.borrowingHistory.map((loan, index) => (
                <div key={index} className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <p className="font-medium">{loan.amount.toFixed(4)} SUI</p>
